...
export const AdminProduits = () => {
    ...
    const loadProduits = () => {
        setLoading(true);
        fetch('/api/admin/produits/', { headers: { 'Authorization': `Bearer ${localStorage.getItem('pauline_token')}` } })
            .then(r => r.json())
    ...
    const handleSubmit = async (e) => {
        ...
        const url = editId
            ? '/api/admin/produits/edit/'
            : '/api/admin/produits/add/';

        try {
            const res  = await fetch(url, { method: 'POST', headers: { 'Authorization': `Bearer ${localStorage.getItem('pauline_token')}` }, body: fd });
    ...
    const handleDelete = async (id, nom) => {
        ...
        try {
            const res  = await fetch('/api/admin/produits/delete/', {
                method:  'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('pauline_token')}` },
